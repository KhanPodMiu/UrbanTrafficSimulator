...
void VisualizationEngine::renderVehicleRoute(
    RenderWindow& window,
    const Camera& camera,
    const Vehicle& vehicle) const
{
    ...
    auto toScreen = [this, &camera](const Vector2& worldPos) -> Vector2 {
        Vector2 screen = applyCamera(worldPos, camera);
    ...
    for (size_t i = 0; i < routeIndex && i < route.size(); ++i)
    {
        ...
        drawThickLine(renderer, startS.x, startS.y, endS.x, endS.y, std::max(2.0f, 4.0f * zoom));
    }
    ...
    if (routeIndex < route.size() && route[routeIndex])
    {
        ...
        if (currentDst)
        {
            ...
            drawThickLine(renderer, vehicleS.x, vehicleS.y, roadEndS.x, roadEndS.y, std::max(4.0f, 9.0f * zoom));
            SDL_SetRenderDrawColor(renderer, 0, 229, 255, 255);
            drawThickLine(renderer, vehicleS.x, vehicleS.y, roadEndS.x, roadEndS.y, std::max(2.0f, 4.0f * zoom));
            ...
            if (std::sqrt(dx * dx + dy * dy) > 20.0f * zoom)
            {
                ...
                SDL_SetRenderDrawColor(renderer, 0, 229, 255, 255);
                drawArrowHead(renderer, midX, midY, dx, dy, std::max(8.0f, 14.0f * zoom));
            }
    ...
    for (size_t i = routeIndex + 1; i < route.size(); ++i)
    {
        ...
        {
            SDL_SetRenderDrawColor(renderer, 0, 229, 255, 110);
            drawThickLine(renderer, lastScreenPos.x, lastScreenPos.y, startS.x, startS.y, std::max(4.0f, 9.0f * zoom));
            SDL_SetRenderDrawColor(renderer, 0, 229, 255, 255);
            drawThickLine(renderer, lastScreenPos.x, lastScreenPos.y, startS.x, startS.y, std::max(2.0f, 4.0f * zoom));
        }

        // Road segment
        SDL_SetRenderDrawColor(renderer, 0, 229, 255, 110);
        drawThickLine(renderer, startS.x, startS.y, endS.x, endS.y, std::max(4.0f, 9.0f * zoom));
        SDL_SetRenderDrawColor(renderer, 0, 229, 255, 255);
        drawThickLine(renderer, startS.x, startS.y, endS.x, endS.y, std::max(2.0f, 4.0f * zoom));
        ...
        SDL_SetRenderDrawColor(renderer, 0, 229, 255, 255);
        fillCircle(renderer, static_cast<int>(std::lround(startS.x)), static_cast<int>(std::lround(startS.y)), wayRadius);
    ...
    if (!route.empty() && route[0])
    {
        ...
        if (startInt)
        {
            ...
            drawCircle(renderer, static_cast<int>(std::lround(startS.x)), static_cast<int>(std::lround(startS.y)), outerR);
            drawCircle(renderer, static_cast<int>(std::lround(startS.x)), static_cast<int>(std::lround(startS.y)), outerR - 1);
            SDL_SetRenderDrawColor(renderer, 46, 204, 113, 255);
            fillCircle(renderer, static_cast<int>(std::lround(startS.x)), static_cast<int>(std::lround(startS.y)), innerR);
        }
    }
    ...
    if (destInt)
    {
        ...
        fillCircle(renderer, cx, cy, outerR);

        SDL_SetRenderDrawColor(renderer, 255, 23, 68, 255);
        drawCircle(renderer, cx, cy, outerR);
        drawCircle(renderer, cx, cy, midR);

        SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
        fillCircle(renderer, cx, cy, innerR);
    ...
    const std::shared_ptr<Intersection>& selectedIntersection) const
{
    ...
    {
        renderVehicleRoute(window, camera, *selectedVehicle);
